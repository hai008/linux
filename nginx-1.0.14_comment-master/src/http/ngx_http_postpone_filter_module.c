...
ngx_http_postpone_filter(ngx_http_request_t *r, ngx_chain_t *in)
{
    ...
    if (r != c->data) {							//r不是当前正在处理的请求
        ...
        if (in) {
            ngx_http_postpone_filter_add(r, in);//数据加入到父请求链表末尾
    ...
    if (in) {								//有需要延后发送的数据
        ngx_http_postpone_filter_add(r, in);//将数据加入到父请求链表末尾
    }

    do {									//循环处理现有的待发数据
        pr = r->postponed;					//获取头结点

        if (pr->request) {					//节点是子请求

            ngx_log_debug2(NGX_LOG_DEBUG_HTTP, c->log, 0,
                           "http postpone filter wake \"%V?%V\"",
                           &pr->request->uri, &pr->request->args);

            r->postponed = pr->next;		//移除头结点
    ...
    } while (r->postponed);			//直到所有数据节点处理完毕
