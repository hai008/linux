...
static ngx_command_t  ngx_http_memcached_commands[] = {
    ...
    { ngx_string("memcached_pass"),			//启动upstream框架
...
ngx_http_memcached_handler(ngx_http_request_t *r)
{
    ...
    rc = ngx_http_discard_request_body(r);					//丢弃请求体,因为访问memcached服务器不需要请求体
    ...
    if (ngx_http_upstream_create(r) != NGX_OK) {			//初始化upstream
        return NGX_HTTP_INTERNAL_SERVER_ERROR;
    }
    ...
    ngx_http_upstream_init(r);								//启动upstream

    return NGX_DONE;										//必须返回NGX_DONE
}
