...
ngx_http_header_filter(ngx_http_request_t *r)
{
    ...
    if (r->header_sent) {//为1表示已经发送过了
        return NGX_OK;
    }

    r->header_sent = 1;

    if (r != r->main) {//main派生子请求的原始请求
        return NGX_OK;
    }
    ...
    if (r->method == NGX_HTTP_HEAD) {//请求方法
        r->header_only = 1;
    }

    if (r->headers_out.last_modified_time != -1) {
        if (r->headers_out.status != NGX_HTTP_OK//http响应状态
    ...
    if (r->headers_out.status_line.len) {//status_line http响应状态行
