...
ngx_http_core_generic_phase(ngx_http_request_t *r, ngx_http_phase_handler_t *ph)
{
    ...
    rc = ph->handler(r);
	/* phase处理完毕，将r->phase_handler指向下一个phase的第一个handler，返回NGX_AGAIN*/
    if (rc == NGX_OK) {
        r->phase_handler = ph->next;
        return NGX_AGAIN;
    }
	/* 继续处理本phase。handler因为某种原因没有执行，继续执行其他的handler,返回NGX_AGAIN */
    if (rc == NGX_DECLINED) {
        r->phase_handler++;
        return NGX_AGAIN;
    }
    ...
    ngx_http_phase_handler_t *ph)
{
    ...
    r->phase_handler++;
	return NGX_AGAIN; /* find config的checker可能会被执行多次，当rewrite成功后，会修改uri参数，需要重新匹配location。*/
}
...
// 这个是post access phase的checker，用于对access phase做收尾处理
ngx_int_t
    ...
    ngx_http_post_subrequest_t *ps, ngx_uint_t flags)
{
    ...
    if (r->main->subrequests == 0) {									//子请求数目达到上限
        ...
        r->main->subrequests = 1;										//子请求创建失败，返回错误
        return NGX_ERROR;
    }

    sr = ngx_pcalloc(r->pool, sizeof(ngx_http_request_t));				//创建子请求对象
    if (sr == NULL) {
        return NGX_ERROR;
    }
    ...
    sr->ctx = ngx_pcalloc(r->pool, sizeof(void *) * ngx_http_max_module);//创建模块的ctx存储空间
    if (sr->ctx == NULL) {
        return NGX_ERROR;
    }
    ...
    sr->method = NGX_HTTP_GET;				//请求方法默认为GET
    sr->http_version = r->http_version;		//拷贝HTTP版本信息

    sr->request_line = r->request_line;		//复用原请求行
    sr->uri = *uri;							//改写子请求的URI
    ...
    sr->unparsed_uri = r->unparsed_uri;				//复用原始URI
    sr->method_name = ngx_http_core_get_method;		//请求方法默认为GET
    sr->http_protocol = r->http_protocol;			//复用原请求协议

    ngx_http_set_exten(sr);							//改写自请求的拓展名

    sr->main = r->main;								//设置主请求
    sr->parent = r;									//设置父请求
    sr->post_subrequest = ps;						//子请求结束后的回调函数
    ...
    pr = ngx_palloc(r->pool, sizeof(ngx_http_postponed_request_t));//子请求的数据对象
    if (pr == NULL) {
        return NGX_ERROR;
    }
